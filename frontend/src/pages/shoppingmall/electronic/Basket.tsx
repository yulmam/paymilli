...
const SHOPPING_MALL = "Samsung Mall";
...
export default function ShoppingBasket() {
  ...
  const handleNavigation = () => {
    navigate("/mall/electronic/order");
  };

  return (
    <S.container>
      <HeaderComponent mall="electronic" />
      <S.pageNav> {"Home > 장바구니"}</S.pageNav>
      {!cartList || cartList.length === 0 ? (
        <EmptyCart mall="electronic" />
      ) : (
        <S.table>
          <S.caption>
            <S.tabListText>
              ...
              <S.DeliveryContainer>
                {totalCost < THE_FREE_SHIPPING_MINIMUM_AMOUNT ? (
                  <S.text2>
                    {THE_FREE_SHIPPING_MINIMUM_AMOUNT - totalCost}원 추가시
                    무료배송
                  </S.text2>
                ) : (
          ...
          <S.tbody>
            {cartList.map((item) => (
              <S.tr key={item.id}>
                ...
                <S.th>
                  <S.priceContainer>
                    <S.Price>{formatCurrency(item.price)}원</S.Price>
                    <S.OriPrice>{formatCurrency(item.oriPrice)}원</S.OriPrice>
                  </S.priceContainer>
                </S.th>
                <S.th>
                  <S.countContainer>
                ...
                <S.th>{formatCurrency(item.price * item.count)}원</S.th>
              </S.tr>
            ))}
          </S.tbody>
          ...
          <S.tfoot>
            <S.priceDescription>
              <S.text>총 금액</S.text>
              <S.boldText>{formatCurrency(totalCost)}원</S.boldText>
              <S.symbol>+</S.symbol>
              <S.text>배송비</S.text>
              <S.boldText>
                {totalCost >= 30000 ? 0 : THE_DELIVERY_CHARGE}원
              </S.boldText>
              <S.text2>{"(30만원이상 구매시 무료배송)"}</S.text2>
              <S.symbol>=</S.symbol>
              <S.text>결제 금액</S.text>
              <S.boldText>
                {formatCurrency(
